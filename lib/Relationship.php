...
abstract class AbstractRelationship implements InterfaceRelationship
{
	...
	public function __construct($options = []) {
		...
		$this->attribute_name = strtolower(Inflector::instance()
		                                            ->variablize($this->attribute_name));
	 ...
	 * the related table by PK/FK and attaches the array of returned relationships to the appropriately named
	 * relationship on
	 * $models.
	 *
	 * @param Table $table
	 ...
	 * @return void
	 */
	protected function query_and_attach_related_models_eagerly(Table $table, $models, $attributes, $includes = [],
	                                                                 $query_keys = [], $model_values_keys = []) {
		$values = [];
		...
		if (!empty($options['through'])) {
			...
			if (!isset($options['class_name'])) {
				...
				$relation = $class::table()
				                  ->get_relationship($options['through']);
	...
	protected function set_class_name($class_name) {
		...
		$reflection = Reflections::instance()
		                         ->add($class_name)
		                         ->get($class_name);
	...
	protected function create_conditions_from_keys(Model $model, $condition_keys = [], $value_keys = []) {
		...
		$conditions = SQLBuilder::create_conditions_from_underscored_string(Table::load(get_class($model))->conn,
	...
	public function construct_inner_join_sql(Table $from_table, $using_through = false, $alias = null) {
		if ($using_through) {
			...
			$from_table_name = Table::load($this->class_name)
...
class HasMany extends AbstractRelationship
{
	...
	public function load(Model $model) {
		...
		if (!isset($this->initialized)) {
			if ($this->through) {
				...
				if (!($through_relationship = $this->get_table()
				                                   ->get_relationship($this->through))) {
	...
	private function get_foreign_key_for_new_association(Model $model) {
		$this->set_keys($model);
		$primary_key = Inflector::instance()
		                        ->variablize($this->foreign_key[0]);
