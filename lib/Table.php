...
class Table
{
	...
	public static function clear_cache($model_class_name = null): void {
	...
	public function __construct($class_name) {
		$this->class = Reflections::instance()
		                          ->add($class_name)
		                          ->get($class_name);
	...
	public function reestablish_connection($close = true): Connection {
	...
	public function options_to_sql($options): SQLBuilder {
	...
	public function find_by_sql($sql, $values = null, $readonly = false, $includes = null): array {
		$this->last_sql = $sql;
		...
		$self = $this;
		while (($row = $sth->fetch())) {
			$cb = static function () use ($row, $self) {
	 ...
	 * @return void
	 */
	private function execute_eager_load($models = [], $attrs = [], $includes = []): void {
	 ...
	 */
	public function get_relationship($name, $strict = false): HasMany|BelongsTo|HasOne|null {
	 ...
	 */
	private function add_relationship($relationship): void {
	...
	private function get_meta_data(): void {
		...
		$conn = $this->conn;
		$this->columns = Cache::get("get_meta_data-$table_name", static function () use ($conn, $table_name) {
	...
	private function set_table_name(): void {
		if (($table = $this->class->getStaticPropertyValue('table', null))
			...
			$this->table = Inflector::instance()
			                        ->tableize($this->class->getName());
	...
	/**
	 * @deprecated Model.php now checks for get|set_ methods via method_exists so there is no need for declaring static
	 *             g|setters.
	 */
